...
Ymodem::Code YmodemFileTransmit::callback(Status status, uint8_t *buff, uint32_t *len)
{
    switch(status)
    {
        ...
        case StatusTransmit:
        {
            uint64_t dataLen = fileSize - fileCount;
...
//                }
                ...
                if (dataLen > YMODEM_PACKET_1K_SIZE)
                {
                    fileCount += file->read((char *)buff, YMODEM_PACKET_1K_SIZE);

                    *len = YMODEM_PACKET_1K_SIZE;
                }
                else if (dataLen < YMODEM_PACKET_SIZE)
                {
                    uint64_t wordCount = file->read((char *)buff, YMODEM_PACKET_SIZE);

                    fileCount += wordCount;

                    memset(&buff[wordCount], 0x1a, YMODEM_PACKET_SIZE - wordCount);
